...
class LabelDecoder(anCoder):
    ...
    def forward(self, input):
        reconstructed, debug = self.decode(input)
    ...
    def decode(self, input):
        ...
        p = torch.sigmoid(h[:, self.p_dims[-1]:])

        m = torch.distributions.NegativeBinomial(r, logits=p)
        return m, (r, p)
