...
class Actnormlayer(nn.Module):
	#intialize data dependent weights, bias
	...
	def initialize_parameters(self, x):
		with torch.no_grad():
			bias  = cpd_mean(x.clone(), dim =[0],keepdim = True)*-1.0
			vars  = cpd_mean((x.clone()+bias)**2,  dim = [0], keepdim = True)
	...
	def _scale(self,x,logdet=0.,reverse=False):
		_shape = list(x.size())[-1]
		if not reverse:
			 x = x*torch.exp(self.logs)
		else:
			x = x*torch.exp(-self.logs)

		dlogdet = cpd_sum(self.logs)
		if reverse:
			dlogdet = -1.0* dlogdet
...
class FlowStep_fc(nn.Module):
	def __init__(self,in_channels, out_channels, hidden_channels,actnorm_scale,cond_dim,coupling):
		super().__init__()
		...
		if coupling == 'linear':
			...
			self.actnorm = Cond_ActNorm(in_channels, hidden_channels,cond_dim) #Actnormlayer(in_channels) #Cond_ActNorm(in_channels, hidden_channels,cond_dim)
			...
			self.switch = Switch()#Shuffle(512)
...
class FlowNet_fc(nn.Module):
	def __init__(self, image_shape, hidden_channels, K, L,
				 actnorm_scale=1.0,cond_dim=2,coupling='linear'):
		"""
							 K                                      K
		...
		for i in range(L):
			...
			self.output_shapes.append([-1, W])
			# 2. K FlowStep
			for _ in range(K):
				self.layers.append(
					FlowStep_fc(in_channels=W,out_channels=W, hidden_channels=hidden_channels, actnorm_scale=actnorm_scale,cond_dim=cond_dim,coupling=coupling))
...
class GlowLatent(nn.Module):
	...
	def __init__(self,batch_size,input_dim,hidden_channels=2048,K=16,gaussian_dims=(1024-700),gaussian_var=0.1,cond_dim=None,coupling='linear'):
		super().__init__()
		self.flow = FlowNet_fc(image_shape= input_dim, # dim of z_t' or z_i'
							hidden_channels=hidden_channels,
							K=K,
							L=1,
							actnorm_scale=1.0,
							cond_dim=cond_dim, # dim of z_s
							coupling=coupling
							)#.to(device)
		C = int(self.flow.output_shapes[-1][1])
		self.learn_top = nn.Linear(input_dim, 2*(input_dim))
	...
	def forward(self, x=None, z_im=None, z=None, cond=None, eps_std=None, reverse=False, tstep = -1):	
		...
		#print('cond shape: ' + str(cond.size()))		
		
		if not reverse:
			#code for forward pass in normalizing flow
			#x = z_t' or z_i' in normal flow pass
	...
	def normal_flow(self, x, z_im, cond=None):
		...
		pixels = x_shape[1]
		z = x #+ torch.rand( x.size(), device=device )*(1. / 256.)
		logdet = torch.zeros_like(x[:, 0])
		logdet = logdet#+float(-np.log(256.)*x_shape[1])
		# encode
